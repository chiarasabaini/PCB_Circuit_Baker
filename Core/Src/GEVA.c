...
#define GEVA_CHAR_BUFFER_SIZE (C_HEIGHT_L * C_WIDTH_L) // arbitrarily large buffer (a spanne)
    ...
    if (n_pos[X] >= buf->size_x || n_pos[X] < 0 || n_pos[Y] >= buf->size_y || n_pos[Y] < 0)
        ...
        uint16_t new_chr[GEVA_CHAR_BUFFER_SIZE]={0};
        ...
        for (uint8_t i = 0; i < font_height; i++) {
            ...
            temp = c[font_height - 1 - i] >> shifter_a;  /// temp = new_chr[font_height - 1 - i] >> shifter_a   rotation doesnt work probably
 ...
 * 4 starting point out of scope
*/
uint8_t put_string(uint8_t pos_x, uint8_t pos_y, char *chr, enum fonts_size ff, video_buffer* buf) {
...
uint8_t put_vbc(vbc_file file, uint8_t x, uint8_t y, video_buffer* buf) {
    ...
    if (file.header.id != 0xA7)
        ...
        memmove(&buf->vid_buf, file.body, 1024 );
        ...
        for (uint8_t row = 0; row < file.header.height ; row++) {
            buffer_byte_ptr = start_byte_ptr - (buf->byte_col_cnt * row);
            memmove(&buf->vid_buf[buffer_byte_ptr], &file.body[((file.header.height - 1) * byte_width) - row * byte_width], byte_width);
        }
    }
    return error;
}
...
uint8_t update_chart(float new_data, data_container* c, video_buffer* buf){
    ...
    case SCATTER:
        ...
        break;
    case LINE_CHUNK:
        error = update_LINE_at_once(c, buf);
        break;
    default:
        break;
...
uint8_t update_LINE(data_container* c, video_buffer* buf) {
    ...
    if (c->dim_x == 0 || c->max <= c->min)
        ...
        if (x_cnt == 0) {
            ...
            for (uint8_t i = 0; (dx*i) <= c->dim_x; i++) //draw x data markers
...
/* Draw line like chart similar to line but data are taken from an array and displayed all at once
 * return error code
 * error code list:
 * 1 data out of range
 * 2 dimension mismatch
...
uint8_t update_LINE_at_once(data_container* c, video_buffer* buf){
    ...
    if (c->dim_x == 0 || c->max <= c->min)
        error = 3;
            ...
            for (uint8_t i = 1; i < c->xy_entries.n_ent; i++){

                value = (c->xy_entries.y[i] * c->dim_y_pos / c->max);
                old_value = (c->xy_entries.y[i-1] * c->dim_y_pos / c->max);
                dx = c->xy_entries.x[i] * c->dim_x / c->max_x;
                //put_line(c->xy_entries.x[i-1] + c->o_x, c->xy_entries.y[i-1]+c->o_y, c->xy_entries.x[i],c->xy_entries.y[i],buf);
                error = put_line(x, c->o_y +(uint8_t)old_value, x + dx, c->o_y + value, buf);               
                x += dx;
            } 
    }
    return error;
